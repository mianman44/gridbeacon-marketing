...
(async () => {
  let html = fs.readFileSync(path.join(root,'components/marketing/stitch-how.html'),'utf8');
  const urls = [...new Set([...html.matchAll(/src="(https:[^"]+)"/g)].map(m=>m[1]))];
  fs.mkdirSync(path.join(root,'public/how-assets'),{recursive:true});
  for (const [i,url] of urls.entries()) {
    ...
    const local=`/how-assets/asset-${i}.${ext}`;
  ...
  for (const [i,url] of files.entries()) {
    const local=`/how-assets/font-${i}.${url.split('.').pop()}`;
    fs.writeFileSync(path.join(root,'public',local),Buffer.from(await (await get(url)).arrayBuffer()));
    css=css.replaceAll(url,local);
  }
  fs.writeFileSync(path.join(root,'public/how-fonts.css'),css);
  fs.writeFileSync(path.join(root,'components/marketing/stitch-how.html'),html);
  fs.writeFileSync(path.join(root,'components/marketing/stitch-how-markup.ts'),'// Reviewed static Stitch homepage; no executable export scripts.\nexport const howMarkup = '+JSON.stringify(html)+';\n');
